...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        // team_1.
        if let Some(new_scores) = scores.get_mut(&team_1_name)
        {
        ...
        if let Some(new_scores) = scores.get_mut(&team_2_name)
        {
            new_scores.goals_scored += team_2_score;
            new_scores.goals_conceded += team_1_score;
        } else {
